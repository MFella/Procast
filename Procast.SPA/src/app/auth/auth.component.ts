...
export class AuthComponent implements OnInit {
  ...
  private static readonly EMAIL_PATTERN: string = '^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,4}$';
  private static readonly PASSWORD_PATTERN: string = '^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{8,}$';
  ...
  isLoginFormInputInvalid(formControlName: keyof LoginFormControlTypes): boolean {
    const formControl: AbstractControl | null = this.loginForm.get(formControlName);
    return !formControl?.hasError('required') && !!formControl?.hasError('pattern');
  }

  isRegisterFormInputInvalid(formControlName: keyof RegisterFormControlTypes): boolean {
    const formControl: AbstractControl | null = this.registerForm.get(formControlName);
    return !formControl?.hasError('required') && Object.keys(formControl?.errors ?? {})?.length > 1
  }
  ...
  getPasswordViolations(): Array<string> {
    ...
    const repeatPasswordControl: AbstractControl | null = this.registerForm.get('repeatPassword');

    if (passwordControl?.hasError('maxLength') || repeatPasswordControl?.hasError('maxLength')) {
      passwordViolations.push('is too long(max length is 64 signs)');
    }

    if (passwordControl?.hasError('minLength') || repeatPasswordControl?.hasError('minLength')) {
      passwordViolations.push('is too short(min length is 8 signs)');
    }

    if (passwordControl?.hasError('pattern') || repeatPasswordControl?.hasError('pattern')) {
      passwordViolations.push('should contains at least one uppercase and lowercase letter, and one number');
    }
  ...
  submitRegisterUser(): void {
    ...
    Single.from(this.authService.registerUser(userToRegister))
      .subscribe({
        next: () => {
          this.messageService.notify('PC_AUTH_SIGN_UP_SUCCESS', 'success');
        },
  ...
  submitLoginUser(): void {
    const userToLogin = this.getUserToLogin();
  }

  private setForms(): void {
    this.loginForm = this.formBuilder.group<LoginFormControlTypes>({
      email: new FormControl('', [Validators.required, Validators.pattern(AuthComponent.EMAIL_PATTERN)]),
    ...
    this.registerForm = this.formBuilder.group<RegisterFormControlTypes>({
      ...
      email: new FormControl('', [Validators.pattern(AuthComponent.EMAIL_PATTERN)]),
      password: new FormControl('', this.passwordValidators),
  ...
  private isPasswordsEqual(): ValidatorFn {
    return (controls: AbstractControl): ValidationErrors | null => {
      ...
      const repeatPassword: AbstractControl | null = controls.get('repeatPassword');

      return password?.value === repeatPassword?.value ? null : { notMatch: true };
    }
  }
  ...
  // asyncValidator if email already exists in db
}
